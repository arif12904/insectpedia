...
class Model:
    ...
    def predict_from_image(self, image_file):
        """
        Preprocessing dan prediksi untuk model CNN modern (input RGB 256x256).
        Input:
            image_file : path atau file image
        Output:
            list berisi kelas prediksi
        """
        ...
        # Tambahkan batch dimension (1, 256, 256, 3)
        image_array = np.expand_dims(image_array, axis=0)

        if self.model_type == 'keras':
            prediction = self.model.predict(image_array)
            prediction_class = np.argmax(prediction, axis=1)
            return prediction_class.tolist()
